...
module Make (Spec_ : SPEC) : CFG with module Spec = Spec_ = struct
  ...
  module NTSet = MySet (struct
    type t = nt

    let compare = compare_nt
  end)

  module NTMap = MyMap (struct
    type t = nt

    let compare = compare_nt
  end)

  module ProdSet = MySet (struct
    type t = prod * symbol list

    let compare = compare_prod
  end)

  module ProdMap = MyMap (struct
    type t = prod * symbol list
  ...
  let maybe_get_prods gr nt =
    try NTMap.find nt gr with Not_found -> ProdSet.empty
  ...
  let sym_derivable nts = function NT nt -> NTMap.mem nt nts | _ -> true
  ...
  let rec calc_syms_deriv n live_nts = function
    ...
    | NT nt :: syms -> (
        let new_d =
          try Some (max (NTMap.find nt live_nts + 1) n) with Not_found -> None
        in
        match new_d with
        | None -> ()
  ...
  let coll_live_info old_live nt prods ((live_nts, nonlive_nts) as live_info) =
    try
      calc_prods_deriv old_live prods;
      live_info
  ...
  let rec split_live_info ((live_nts, nonlive_nts) as live_info) =
    let ((new_live_nts, new_nonlive_nts) as new_live_info) =
      NTMap.fold (coll_live_info live_nts) nonlive_nts live_info
    in
    if
  ...
  let deriv_prods live_nts ((_, syms) as prod) prods =
    try
      calc_syms_deriv 1 live_nts syms;
      prods
    with Found n -> ProdMap.add prod n prods

  let derive_nts gr live_nts nt n =
    let prods =
      ProdSet.fold (deriv_prods live_nts) (NTMap.find nt gr) ProdMap.empty
    in
  ...
  let coll_reachable_sym gr acc = function
    | T _ -> acc
    | NT nt -> (
        try NTMap.add nt (NTMap.find nt gr) acc with Not_found -> acc)
  ...
  let rec get_unreachable gr root_nts =
    if NTMap.is_empty root_nts then gr
      ...
      let reachable_nts =
        NTMap.fold (coll_reachable_nt new_gr) root_nts NTMap.empty
      in
  ...
  let prune_live_prods gr nt prod _ acc =
    if ProdSet.mem prod (NTMap.find nt gr) then acc else ProdMap.remove prod acc
  ...
  let rec bnd_descend levels gr reached_nts n =
    if n <= 0 || NTSet.is_empty reached_nts then levels
      ...
      let ((_, new_reached_nts) as this_level) =
        NTSet.fold (bnd_descend_nt gr) reached_nts (TSet.empty, NTSet.empty)
      in
  ...
  let bnd_ascend_nt gr kept_nts nt nts =
    ...
    if ProdSet.is_empty kept_prods then nts else NTMap.add nt kept_prods nts

  let cleanup_sym (ts, nts) =
    if TSet.is_empty ts && NTMap.is_empty nts then raise Exit
    else function
      | T t -> (TSet.remove t ts, nts) | NT nt -> (ts, NTMap.remove nt nts)
  ...
  let rec cleanup_levels kept_nts = function
    ...
    | ((ts, nts) as level) :: rest as levels -> (
        try
          let bad_ts, bad_nts =
            NTMap.fold (fun _ -> ProdSet.fold cleanup_prod) kept_nts level
          in
          ...
          if NTMap.is_empty bad_nts then
            if TSet.is_empty bad_ts then levels else (good_ts, nts) :: rest
