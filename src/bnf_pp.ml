...
let rec pp_prod ppf = function
  ...
  | h :: t ->
      fprintf ppf "%s@ " (string_of_symbol h);
      pp_prod ppf t

let pp_prods ppf =
  before_first ProdSet.iter
    (fun _ -> fprintf ppf "@ @[| ")
    (pp_close_box ppf)
    (fun (_, sym_list) -> fprintf ppf "@[%a@]" pp_prod sym_list)

let pp_live_prods ppf =
  before_first ProdMap.iter
    (fun _ -> fprintf ppf "@ @[| ")
    (pp_close_box ppf)
...
let pp_live_nt ppf nt (d, prods) =
  fprintf ppf "@[<0>%s (%d) @[<1>:= %a .@]@]" (string_of_nt nt) d pp_live_prods
    prods
...
let pp_ts ppf =
  before_first TSet.iter force_newline nop (fun t ->
      pp_print_string ppf (string_of_t t))

let pp_nts ppf =
  before_first NTSet.iter force_newline nop (fun nt ->
      pp_print_string ppf (string_of_nt nt))
